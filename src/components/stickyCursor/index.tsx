"use client";
import { useEffect, useState, useRef, Ref, ReactNode, RefObject } from "react";
import {
  motion,
  useMotionValue,
  useSpring,
...
export default function StickyCursor({
  ...
  const manageMouseMove = (e: MouseEvent) => {
    ...
    const { left, top, height, width } =
      stickyElement?.current?.getBoundingClientRect() as DOMRect;
  ...
  useEffect(() => {
    (stickyElement.current as HTMLDivElement).addEventListener(
      "mouseenter",
      manageMouseOver
    );
    (stickyElement.current as HTMLDivElement).addEventListener(
    ...
    return () => {
      (stickyElement.current as HTMLDivElement).removeEventListener(
        "mouseenter",
        manageMouseOver
      );
      (stickyElement.current as HTMLDivElement).removeEventListener(
